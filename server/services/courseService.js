...
async function searchCatalog(keyword) {
  ...
  const [courses] = await pool.query(
    'SELECT id, name, professor, credits FROM courses WHERE name LIKE ? OR id LIKE ? ORDER BY id',
    [like, like]
  );
...
async function listMyCourses(studentId, { year, semester }) {
  let sql = `
    SELECT sc.id, sc.course_id, c.name, c.professor, c.credits, sc.year, sc.semester,
...
async function addMyCourse(studentId, { courseId, year, semester }) {
  const [result] = await pool.query(
    'INSERT INTO student_courses (student_id, course_id, year, semester) VALUES (?, ?, ?, ?)',
...
async function getTimetable(studentId, { year, semester }) {
  const [rows] = await pool.query(
    `SELECT cs.day, cs.period, c.id AS course_id, c.name
     FROM student_courses sc
     JOIN courses c ON c.id = sc.course_id
...
async function getSummary(studentId) {
  const [rows] = await pool.query(
    `SELECT sc.year, sc.semester, c.credits, sc.gpa, sc.letter_grade
     FROM student_courses sc
     JOIN courses c ON c.id = sc.course_id
